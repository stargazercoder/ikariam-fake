-- Migration: Environment guard for production timers
-- When app.environment = 'production', revert the speed-up timers from migration
-- 20260312000007 back to their original production values:
--   resource-tick: every 1 minute → every 5 minutes
--   battle turn interval: 10 seconds → 5 minutes
--   battle start delay: 10 seconds → 5 minutes
--
-- When app.environment IS DISTINCT FROM 'production' (NULL or 'development'),
-- the speed-up timers from migration 20260312000007 remain active.
--
-- Usage: set app.environment = 'production' in Supabase project settings
--        (Database → Settings → Custom Config → app.environment = production)
...
BEGIN
  IF current_setting('app.environment', true) = 'production' THEN
    ...
    -- ============================================================
    -- 1. Reschedule resource-tick: every 1 minute → every 5 minutes
    -- ============================================================
    ...
    -- ============================================================
    -- 2. Restore resolve_battles() with 5-minute turn interval
    --    (replaces the 10-second version from migration 20260312000007)
    -- ============================================================
    ...
    BEGIN
      ...
      LOOP
        ...
        ELSE
          -- Battle continues: advance turn — 5 MINUTES (production interval)
          UPDATE public.battles
    ...
    -- ============================================================
    -- 3. Restore process_arrivals() with 5-minute battle start delay
    --    (replaces the 10-second version from migration 20260312000007)
    -- ============================================================
      ...
      LOOP
        ...
        ELSE
          ...
          IF NOT v_active_battle THEN
            ...
            -- First turn resolves 5 MINUTES from now (production interval)
