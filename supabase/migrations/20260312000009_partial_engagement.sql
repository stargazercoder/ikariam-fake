-- Migration: Partial army engagement in resolve_battles() (CMBT-02)
--
-- Replaces resolve_battles() with a version where only 30% of each unit type
-- engages per battle turn. Both naval and land phases use the engagement fraction.
--
-- This migration handles BOTH environments:
--   - Production (app.environment = 'production'): 5-minute turn interval
--   - Development (NULL or 'development'): 10-second turn interval (speed-up)
--
-- The v_engagement_fraction constant must stay in sync with the Dart test constant
-- engagementFraction = 0.30 in test/unit/combat_engagement_test.dart.
...
DECLARE
  ...
  -- Engagement fraction: only 30% of each unit type participates per turn.
  -- Mirrors engagementFraction = 0.30 in test/unit/combat_engagement_test.dart.
  ...
  v_kv                      RECORD;
BEGIN
  -- Determine turn interval based on environment
  IF current_setting('app.environment', true) = 'production' THEN
    v_turn_interval := INTERVAL '5 minutes';
  ELSE
    v_turn_interval := INTERVAL '10 seconds';
  END IF;
  ...
  LOOP
    ...
    -- NAVAL PHASE
    -- Attack/defense summation uses engaged subset (30% of each unit type).
    -- GREATEST(1, FLOOR(...)) ensures at least 1 unit engages when qty > 0.
    ...
    FOREACH v_unit_type IN ARRAY v_naval_types LOOP
      ...
      v_def_engaged := GREATEST(1, FLOOR(v_def_qty * v_engagement_fraction));
      IF v_def_qty > 0 THEN
        v_def_naval_attack  := v_def_naval_attack  + v_def_engaged * COALESCE((v_unit_attack  ->> v_unit_type)::numeric, 0);
        v_def_naval_defense := v_def_naval_defense + v_def_engaged * COALESCE((v_unit_defense ->> v_unit_type)::numeric, 0);
      END IF;
    END LOOP;
    ...
    ELSE
      ...
      -- Casualty application uses full army qty (not engaged).
      -- loss_ratio already reflects engaged subset strength.
      -- LEAST clamp ensures casualties never exceed total army size.
      FOREACH v_unit_type IN ARRAY v_naval_types LOOP
        v_att_qty := COALESCE((v_att_units ->> v_unit_type)::numeric, 0);
        IF v_att_qty > 0 THEN
    ...
    IF NOT v_battle_ended THEN
      ...
      FOR v_kv IN SELECT key, value FROM jsonb_each_text(v_att_units) LOOP
        IF NOT (v_kv.key = ANY(v_naval_types)) THEN
          ...
          v_att_engaged := GREATEST(1, FLOOR(v_att_qty * v_engagement_fraction));
          v_att_land_attack  := v_att_land_attack  + v_att_engaged * COALESCE((v_unit_attack  ->> v_kv.key)::numeric, 0);
          v_att_land_defense := v_att_land_defense + v_att_engaged * COALESCE((v_unit_defense ->> v_kv.key)::numeric, 0);
        END IF;
      END LOOP;
      ...
      FOR v_kv IN SELECT key, value FROM jsonb_each_text(v_def_units) LOOP
        IF NOT (v_kv.key = ANY(v_naval_types)) THEN
          ...
          v_def_engaged := GREATEST(1, FLOOR(v_def_qty * v_engagement_fraction));
          v_def_land_attack  := v_def_land_attack  + v_def_engaged * COALESCE((v_unit_attack  ->> v_kv.key)::numeric, 0);
          v_def_land_defense := v_def_land_defense + v_def_engaged * COALESCE((v_unit_defense ->> v_kv.key)::numeric, 0) * v_wall_bonus;
        END IF;
      END LOOP;
    ...
    ELSE
      -- Battle continues: advance turn using environment-appropriate interval
      UPDATE public.battles
      SET turn_number    = turn_number + 1,
          next_turn_at   = next_turn_at + v_turn_interval,
          attacker_units = v_att_units,
          defender_units = v_def_units,
          updated_at     = NOW()
      WHERE id = b.id;
