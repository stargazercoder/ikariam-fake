// Barracks stats widget — land unit training grid, active training queue
// countdown, and army roster. Extracted from barracks_screen.dart.
...
/// Barracks building stats — shows the unit training grid for all 8 land
/// unit types, displays the active training queue countdown, and lists the
/// current army roster.
class BarracksStats extends ConsumerStatefulWidget {
  const BarracksStats({
    super.key,
  ...
  @override
  ConsumerState<BarracksStats> createState() => _BarracksStatsState();
}

class _BarracksStatsState extends ConsumerState<BarracksStats> {
  /// Quantity controllers for each land unit type (keyed by UnitType.dbName).
  final Map<String, TextEditingController> _quantityControllers = {};

  /// Land unit types in display order.
  static final List<UnitType> _landUnits =
  ...
  Widget build(BuildContext context) {
    ...
    final landUnits = allUnits.where((u) => !_isNaval(u.unitType)).toList();
    ...
    return Column(
      ...
      children: [
        ...
        ..._landUnits.map((unit) {
          ...
          final isUnlocked = widget.barracksLevel >= requiredLevel;
        ...
        _RosterSection(
          units: landUnits,
          emptyMessage: 'No land units yet. Train some units above!',
        ),
      ],
    );
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (!isUnlocked)
              Text(
                'Requires $buildingName Lv.$requiredLevel',
