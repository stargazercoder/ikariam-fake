// Barracks screen — land unit training, army roster, and dispatch navigation.
...
///
/// Shows 8 land unit types with unlock level gating, a quantity input, and
/// a Train button. Displays active training countdown and current army roster.
class BarracksScreen extends ConsumerStatefulWidget {
  const BarracksScreen({super.key, required this.cityId});

  final String cityId;

  @override
  ConsumerState<BarracksScreen> createState() => _BarracksScreenState();
}

class _BarracksScreenState extends ConsumerState<BarracksScreen> {
  // Quantity controllers for each land unit type (keyed by UnitType.dbName).
  final Map<String, TextEditingController> _quantityControllers = {};

  @override
  void initState() {
    super.initState();
    for (final unit in _landUnits) {
  ...
  /// Land unit types in display order.
  static final List<UnitType> _landUnits = UnitType.values
      .where((u) => !u.isNaval)
      .toList();
  ...
  Widget build(BuildContext context) {
    ...
    // Derive current land units for roster section.
    final landUnits = rosterAsync.whenOrNull(data: (units) => units) ?? [];

    return Scaffold(
      appBar: AppBar(
        title: Text('Barracks (Level $barracksLevel)'),
        actions: [
          IconButton(
            icon: const Icon(Icons.send),
            tooltip: 'Dispatch Units',
            onPressed: () =>
                context.push('/dispatch?cityId=${widget.cityId}'),
          ),
        ],
      ),
            ...
            child: Column(
              ...
              children: [
                ...
                ..._landUnits.map((unit) {
                  ...
                  final isUnlocked = barracksLevel >= requiredLevel;
                ...
                const SizedBox(height: 8),
                _ArmyRosterSection(
                  units: landUnits.where((u) => !_isNaval(u.unitType)).toList(),
                ),
              ],
            ),
          ),
...
class _ArmyRosterSection extends StatelessWidget {
  const _ArmyRosterSection({required this.units});
    ...
    if (units.isEmpty) {
      return Card(
        child: Padding(
          padding: const EdgeInsets.all(16),
          child: Text(
            'No land units yet. Train some units above!',
